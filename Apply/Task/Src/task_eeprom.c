...
static uint8_t Exist_Yes = 'Y';         //存在IP信息标志

static uint8_t Update_YES = 'Y';        //需要更新信息标志
static uint8_t Update_NO = 'N';         //不需要更新信息标志

/**
 * @brief 往EEPROM中保存默认远端和本地的IP信息
 * @param lwipx: lwip控制结构体
 * @note 该函数仅在未写入过AT24C02时执行
 * @retval Null
 */
void Task_EEPROM_WriteDefaultAddrInfo(__lwip_dev *lwipx)
{
    uint8_t IP_Exist_flag = 0;  //IP信息的是否存在标志
    uint8_t PortTemp[2] = {0};  //暂存Port的缓冲区

    /* 读取IP是否存在标志位 */
    OCD_AT24CXX_Read(&EEPROM,IP_EXIST,&IP_Exist_flag,1);
    if(IP_Exist_flag != Exist_Yes)   //AT24C02未使用过，芯片内没有对应的IP信息
    {
        PortTemp[0] = RemotePort >> 8;
        PortTemp[1] = RemotePort & 0xFF;

        /* 将默认的IP信息写入对应地址上 */
        OCD_AT24CXX_Write(&EEPROM,PORT_ADDR,PortTemp,2);
        OCD_AT24CXX_Write(&EEPROM,REMOTEIP_ADDR,lwipx->remoteip,4);
        OCD_AT24CXX_Write(&EEPROM,IP_ADDR,lwipx->ip,4);
        OCD_AT24CXX_Write(&EEPROM,MASK_ADDR,lwipx->netmask,4);
        OCD_AT24CXX_Write(&EEPROM,GW_ADDR,lwipx->gateway,4);
        OCD_AT24CXX_Write(&EEPROM,IP_EXIST,&Exist_Yes,1);
        printf("Write Origin IPAddr Done!\r\n");
    }
}
...
#ifdef PRINTF_DEBUG
    printf("IP_Exist_flag %c\r\n",IP_Exist_flag);
#endif

    /* 如果需要更新则去对应地址读取 */
    if(IP_Exist_flag == Exist_Yes)
    {
